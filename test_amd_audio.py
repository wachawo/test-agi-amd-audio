...
def run_http_server(port: int, server_data: ServerData):
    ...
    with StoppableTCPServer(("0.0.0.0", port), AudioUploadHandler) as httpd:
...
def read_wav_mono(path: str, channel: int = 0):
    ...
    with wave.open(path, "rb") as wf:
        params = wf.getparams()
        logger.info(
            f"WAV: {params.nchannels}ch, {params.framerate}Hz, {params.sampwidth*8}bit"
        )
...
def main():
    ...
    parser.add_argument(
        "-b", "--binary", default="./amd_audio.py", help="Path to amd_audio script"
    )
    ...
    parser.add_argument(
        ...
        "--channel",
        type=int,
        default=CHANNEL_TO_USE,
        help="Audio channel to use (0=Left, 1=Right)",
    )
    parser.add_argument(
        "-i", "--image", action="store_true", help="Generate and show waveform image"
    )
    ...
    server_thread = threading.Thread(
        target=run_http_server, args=(args.port, server_data), daemon=True
    )
    server_thread.start()
    ...
    def feeder():
        ...
        chunk_size = int(
            rate * 2 * 0.1
        )  # 100ms in bytes (2 bytes per sample for 16-bit)
        if chunk_size == 0:
            ...
            with os.fdopen(write_fd, "wb") as f:
                ...
                if raw_audio:
                    ...
                    if first_chunk:
                        ...
                        logger.info(
                            f"First chunk {len(first_chunk):,} bytes sent immediately"
                        )
    ...
    if server_data.received_audio:
        ...
        if idx >= 0:
            ...
            logger.info(
                f"MATCH FOUND! Voice started at {seconds:.3f} seconds in original file."
            )
        else:
            logger.warning(
                "Could not find exact match in original file (maybe processed/transcoded?)"
            )
